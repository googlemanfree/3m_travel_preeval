const CACHE_NAME = '3m-travel-pwa-v9-transparency-footer';
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/@vite/') || url.pathname.startsWith('/src/') || url.pathname.startsWith('/__manus__/')) {
    event.respondWith(fetch(event.request));
    return;
  }
  if (url.pathname.startsWith('/api/')) {
    event.respondWith(fetch(event.request).then((response) => {
      ...
      return response;
    }).catch(() => caches.match(event.request).then((cached) => cached || new Response(JSON.stringify({ offline: true, message: 'Mode hors ligne - Données chargées depuis le cache local.' }), { headers: { 'Content-Type': 'application/json' } }))));
    return;
  }
  ...
  event.respondWith(caches.match(event.request).then((cached) => cached || fetch(event.request).then((response) => caches.open(CACHE_NAME).then((cache) => {
