...
const resources = [
  {
    title: "Canada : comprendre les parcours de résidence permanente",
    description:
      "Repères sur Entrée express, les programmes provinciaux, les programmes régionaux, le parrainage familial et les voies québécoises. Les critères applicables doivent être confirmés auprès d’IRCC.",
    href: "/canada",
    label: "Lire le guide Canada",
    icon: Landmark,
    kind: "Guide 3M",
  },
  {
    title: "Schengen : préparer un séjour de courte durée",
    description:
      "Tourisme, affaires, visite familiale, soins, événement ou transit : le motif du voyage et le consulat compétent déterminent les pièces utiles.",
    href: "/schengen",
    label: "Lire le guide Schengen",
    icon: BookOpen,
    kind: "Guide 3M",
  },
  {
    title: "Règles générales du visa Schengen",
    description:
      "La Commission européenne présente les règles de court séjour, les entrées simples ou multiples et le transit aéroportuaire. Les séjours longs suivent une procédure nationale.",
    href: "https://home-affairs.ec.europa.eu/policies/schengen/visa-policy/applying-schengen-visa_en",
    label: "Consulter la source officielle",
    icon: ShieldCheck,
    kind: "Source officielle",
    external: true,
  },
  {
    title: "Programmes de résidence permanente au Canada",
    description:
      "IRCC maintient la liste des voies disponibles, suspendues ou fermées. Consultez la source officielle avant toute décision et avant de préparer un dossier.",
    href: "https://www.canada.ca/en/immigration-refugees-citizenship/services/immigrate-canada.html",
    label: "Consulter IRCC",
    icon: CalendarClock,
    kind: "Source officielle",
    external: true,
  },
      ...
      <div className="mx-auto max-w-7xl">
          ...
          {resources.map((resource) => {
            ...
            const content = (
              <>
                ...
                <h2 className="mt-6 text-xl font-black text-slate-950">{resource.title}</h2>
                ...
                <span className="mt-6 inline-flex items-center gap-2 text-sm font-black text-blue-700">
                  {resource.label}
                  {resource.external ? <ExternalLink className="h-4 w-4" /> : <ArrowUpRight className="h-4 w-4" />}
                </span>
              </>
            );
            ...
            return resource.external ? (
              ...
              <Link key={resource.title} href={resource.href} className={cardClassName}>
                {content}
              </Link>
            );
          })}
        ...
        <section className="mt-16">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[.18em] text-blue-700">Guides destination</p>
              <h2 className="mt-2 text-3xl font-black tracking-tight text-slate-950">
                Étudier à l’étranger : 10 destinations
              </h2>
              <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
                Chaque article présente le parcours à préparer, les documents à anticiper, les points de vigilance et une source officielle à consulter.
              </p>
            </div>
            <GraduationCap className="hidden h-10 w-10 text-blue-700 sm:block" aria-hidden="true" />
          </div>
          <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {studyDestinationArticles.map((article) => (
              <Link
                key={article.slug}
                href={`/blog/etudes/${article.slug}`}
        ...
        <section className="mt-12 rounded-2xl bg-blue-800 p-8 text-white">
          ...
          <Link
            ...
            className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-white px-5 py-3 text-sm font-black text-blue-900 hover:bg-blue-50"
          >
            Faire évaluer mon profil
          </Link>
        </section>
      </div>
    </main>
