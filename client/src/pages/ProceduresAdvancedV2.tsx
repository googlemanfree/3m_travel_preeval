...
export default function ProceduresAdvanced() {
  ...
  // Regroupe les entrées par pays (un même pays peut avoir plusieurs fiches :
  // travail / études / visiteur) pour n'afficher qu'UNE seule carte par pays,
  // avec un sélecteur interne pour changer de type de visa.
  const groupedCountries = useMemo(() => {
    const groups = new Map<string, typeof filteredCountries>();
    ...
    return Array.from(groups.entries()).map(([name, variants]) => ({
      groupId: variants[0].id.split('-')[0] || name,
      name,
      variants,
    }));
  }, [filteredCountries]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 text-slate-600 font-medium">
          {groupedCountries.length} destination{groupedCountries.length !== 1 ? 's' : ''} trouvée{groupedCountries.length !== 1 ? 's' : ''}
        ...
        >
          <AnimatePresence>
            {groupedCountries.map((group) => {
              const activeVariantId = selectedVariantByGroup[group.name] || group.variants[0].id;
              const country = group.variants.find((v) => v.id === activeVariantId) || group.variants[0];
                      ...
                      {group.variants.length > 1 && (
                        <div className="flex gap-1.5 flex-wrap" onClick={(e) => e.stopPropagation()}>
                          {group.variants.map((v) => (
                            <button
                              key={v.id}
                              onClick={() => setSelectedVariantByGroup((prev) => ({ ...prev, [group.name]: v.id }))}
                            ...
                            >
                              {v.visaType === 'travail' ? '💼 Travail' : v.visaType === 'etudes' ? '🎓 Études' : '🧳 Visiteur'}
                            </button>
                          ))}
                        </div>
                      )}
