...
function StepIndicator({ step, total }: { step: number; total: number }) {
  return (
    <div className="flex items-center justify-center gap-2 mb-8" role="progressbar" aria-label={`Étape ${step} sur ${total}`} aria-valuemin={1} aria-valuemax={total} aria-valuenow={step}>
...
export default function ConsultationBooking() {
  ...
  const goNext = () => {
    ...
    if (step === 3) {
      ...
      const serviceLabel = SERVICE_OPTIONS.find((s) => s.id === form.service)?.label ?? form.service;
      ...
      submitMutation.mutate({
        ...
        message: `Type de consultation : ${serviceLabel}${details ? `\n\n${details}` : ""}`,
      });
      return;
    }
    setStep((s) => s + 1);
      ...
      <div className="mx-auto max-w-xl">
        ...
        <Card className="p-6 md:p-8">
              ...
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Textarea id="cb-message" value={form.message} onChange={(e) => update("message", e.target.value)} placeholder="Décrivez brièvement votre situation ou vos questions…" rows={3} className="mt-1" maxLength={1800} />
                  </div>
          ...
          {submitMutation.error && (
            <p role="alert" className="mt-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{submitMutation.error.message}</p>
          )}
