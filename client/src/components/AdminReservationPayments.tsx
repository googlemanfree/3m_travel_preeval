...
import { CheckCircle2, Clock, Download, FileUp, Mail, RefreshCw, Search, Send, ShieldCheck, Ticket } from "lucide-react";
...
type PaymentRow = {
  ...
  pnrViewedAt?: string | Date | null; pnrDownloadedAt?: string | Date | null; flightData?: unknown;
};
...
export function AdminReservationPayments() {
  ...
  const sendReceiptMutation = trpc.flightBooking.sendPaymentReceiptEmail.useMutation({ onSuccess: () => { toast.success("Reçu de paiement envoyé au client."); void utils.flightBooking.listReservationPayments.invalidate(); }, onError: (err) => toast.error(`Envoi impossible : ${err.message}`) });
  const sendReminderMutation = trpc.flightBooking.sendPnrReminderEmail.useMutation({ onSuccess: () => { toast.success("Relance du billet envoyée au client."); void utils.flightBooking.listReservationPayments.invalidate(); }, onError: (err) => toast.error(`Relance impossible : ${err.message}`) });
  const uploadTicketMutation = trpc.flightBooking.adminUploadPnrDocument.useMutation({
    onSuccess: (data) => {
      setSelectedPayment(null); setPnrReference(""); setAdvisorInitials(""); setPdfFile(null);
      toast.success(data.delivery.emailNotificationDispatched ? "Billet publié dans l’espace client et e-mail envoyé." : "Billet publié dans l’espace client. L’e-mail n’a pas été transmis : utilisez Relancer le billet.");
  ...
  const filtered = ((payments ?? []) as unknown as PaymentRow[]).filter((payment) => {
    ...
    const matchStatus = statusFilter === "all" || (statusFilter === "validated" ? payment.clientValidated : !payment.clientValidated);
    return matchSearch && matchMethod && matchStatus;
  });

  const exportCsv = () => {
    const rows = [["Référence", "Client", "Paiement", "Transaction", "Validation", "Billet", "Date"], ...filtered.map((payment) => [payment.requestRef, payment.candidateEmail, payment.paymentMethod || "", payment.paymentTransactionId || "", payment.clientValidated ? "Oui" : "Non", payment.issuedPdfUrl ? "Émis" : "En attente", new Date(payment.createdAt).toLocaleString("fr-FR")])];
  ...
  const openIssuance = (payment: PaymentRow) => { setSelectedPayment(payment); setPnrReference(payment.pnrReference || ""); setAdvisorInitials(""); setPdfFile(null); };
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-black text-slate-900">Billetterie & remise client</h2><p className="mt-1 text-sm text-slate-500">L’émission publie le PDF dans l’espace client et transmet l’e-mail de remise au client.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => refetch()} className="gap-2"><RefreshCw className="h-4 w-4" /> Actualiser</Button><Button variant="outline" onClick={exportCsv} className="gap-2"><Download className="h-4 w-4" /> CSV</Button></div></div>
    ...
    <Card className="overflow-hidden border-slate-200 bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm"><thead className="bg-slate-50 text-xs font-black uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Dossier / client</th><th className="px-4 py-3">Paiement</th><th className="px-4 py-3">Billet</th><th className="px-4 py-3">Remise</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{isLoading ? <tr><td colSpan={5} className="px-4 py-10 text-center text-slate-500">Chargement des dossiers…</td></tr> : filtered.map((payment) => <tr key={payment.id} className="hover:bg-slate-50"><td className="px-4 py-3"><p className="font-mono font-black text-blue-700">{payment.requestRef}</p><p className="mt-1 max-w-56 truncate text-xs font-semibold text-slate-700">{payment.candidateEmail}</p></td><td className="px-4 py-3"><Badge variant="outline" className={payment.clientValidated ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-800"}>{payment.clientValidated ? "Paiement validé" : "Paiement à confirmer"}</Badge><p className="mt-1 text-xs text-slate-500">{payment.paymentTransactionId || "Transaction non renseignée"}</p></td><td className="px-4 py-3">{payment.issuedPdfUrl ? <div><Badge className="bg-emerald-600 text-white">PDF émis</Badge><p className="mt-1 font-mono text-xs text-emerald-700">{payment.pnrReference || "PNR enregistré"}</p></div> : <Badge variant="outline" className="border-slate-200 text-slate-600">À émettre</Badge>}</td><td className="px-4 py-3">{payment.issuedPdfUrl ? payment.pnrDownloadedAt ? <span className="text-xs font-bold text-emerald-700">Téléchargé</span> : payment.pnrViewedAt ? <span className="text-xs font-bold text-blue-700">Consulté</span> : <span className="text-xs font-bold text-amber-700">À consulter</span> : <span className="text-xs text-slate-500">—</span>}</td><td className="px-4 py-3"><div className="flex justify-end gap-2"><Button type="button" size="sm" onClick={() => openIssuance(payment)} disabled={uploadTicketMutation.isPending} className="bg-emerald-700 text-white hover:bg-emerald-800"><Ticket className="mr-1.5 h-3.5 w-3.5" /> {payment.issuedPdfUrl ? "Remplacer le billet" : "Émettre le billet"}</Button>{payment.issuedPdfUrl && <Button type="button" size="sm" variant="outline" onClick={() => sendReminderMutation.mutate({ sessionToken, requestId: payment.id })} disabled={sendReminderMutation.isPending}><Send className="mr-1.5 h-3.5 w-3.5" /> Relancer</Button>}<Button type="button" size="sm" variant="outline" onClick={() => sendReceiptMutation.mutate({ sessionToken, requestId: payment.id })} disabled={sendReceiptMutation.isPending || !payment.clientValidated}><Mail className="mr-1.5 h-3.5 w-3.5" /> Reçu</Button></div></td></tr>)}{!isLoading && filtered.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-slate-500">Aucun dossier ne correspond aux filtres.</td></tr>}</tbody></table></div></Card>
    <Dialog open={Boolean(selectedPayment)} onOpenChange={(open) => { if (!open && !uploadTicketMutation.isPending) setSelectedPayment(null); }}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle className="flex items-center gap-2"><Ticket className="h-5 w-5 text-emerald-700" /> Émettre et remettre le billet</DialogTitle><DialogDescription>Après validation humaine, le PDF est publié dans l’espace client et un e-mail est transmis à {selectedPayment?.candidateEmail}.</DialogDescription></DialogHeader><div className="space-y-4"><div className="rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-xs text-emerald-900"><ShieldCheck className="mr-1 inline h-4 w-4" /> Le dossier doit avoir sa checklist d’émission complète. Cette étape reste obligatoire.</div><div><Label htmlFor="payment-pnr">Référence PNR / GDS *</Label><Input id="payment-pnr" value={pnrReference} onChange={(event) => setPnrReference(event.target.value)} placeholder="Ex. ABC123" className="mt-1" /></div><div><Label htmlFor="payment-initials">Initiales du conseiller *</Label><Input id="payment-initials" value={advisorInitials} onChange={(event) => setAdvisorInitials(event.target.value.toUpperCase())} maxLength={10} placeholder="Ex. JDM" className="mt-1 font-mono font-bold uppercase" /></div><div><Label htmlFor="payment-ticket">Billet ou PNR final (PDF, 8 Mo maximum) *</Label><Input id="payment-ticket" type="file" accept="application/pdf" onChange={(event) => setPdfFile(event.target.files?.[0] || null)} className="mt-1" />{pdfFile && <p className="mt-1 text-xs text-slate-600"><FileUp className="mr-1 inline h-3.5 w-3.5" /> {pdfFile.name}</p>}</div></div><DialogFooter><Button type="button" variant="outline" onClick={() => setSelectedPayment(null)} disabled={uploadTicketMutation.isPending}>Annuler</Button><Button type="button" onClick={() => void submitIssuance()} disabled={uploadTicketMutation.isPending || !pdfFile || !pnrReference.trim() || !advisorInitials.trim()} className="bg-emerald-700 text-white hover:bg-emerald-800">{uploadTicketMutation.isPending ? "Publication et envoi…" : "Valider, publier et envoyer"}</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}
