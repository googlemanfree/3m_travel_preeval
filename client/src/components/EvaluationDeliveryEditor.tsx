...
import { AlertTriangle, CalendarClock, Clock3, Download, Eye, FilePenLine, FileText, GitCompareArrows, History, Mail, MessageCircle, Printer, Save, Send, ShieldCheck } from "lucide-react";
...
export function EvaluationDeliveryEditor({ sessionToken, sourceRecordId, open, onOpenChange, onCompleted }: Props) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="h-[96vh] max-h-[96vh] w-[calc(100vw-1rem)] max-w-[min(96vw,1600px)] overflow-y-auto p-4 sm:w-[calc(100vw-2rem)] sm:p-6 lg:p-8">
      <DialogHeader>
      ...
      {isLoading || !data ? <div className="py-12 text-center text-sm text-slate-500">Chargement du brouillon d’évaluation…</div> : <div className="grid min-h-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)]">
        <div className="space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2"><Button className="w-full bg-blue-700 hover:bg-blue-800" disabled={sendNow.isPending || saveDraft.isPending || (requiresSecondApproval && data.application.evaluationApprovalStatus !== "approved")} onClick={async () => { if (!(await ensureDraft())) return; if (!currentDraft.advisorValidated) { await validateDraft.mutateAsync({ sessionToken, sourceRecordId, validationComment: "Bilan relu et validé avant diffusion." }); } sendNow.mutate({ sessionToken, sourceRecordId }); }}><Send className="mr-2 h-4 w-4" />{sendNow.isPending ? "Envoi e-mail…" : "Valider et envoyer par e-mail"}</Button><Button type="button" variant="outline" className="w-full border-emerald-300 text-emerald-800 hover:bg-emerald-50" disabled={saveDraft.isPending} onClick={() => void openWhatsAppDraft()}><MessageCircle className="mr-2 h-4 w-4" />Ouvrir WhatsApp avec le bilan</Button></div>
        </div>
