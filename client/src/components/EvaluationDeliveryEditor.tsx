...
export function EvaluationDeliveryEditor({ sessionToken, sourceRecordId, open, onOpenChange, onCompleted }: Props) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="h-[96vh] max-h-[96vh] w-[calc(100vw-1rem)] max-w-[min(96vw,1600px)] overflow-x-hidden overflow-y-auto border-slate-200 bg-slate-50/95 p-3 shadow-2xl sm:w-[calc(100vw-2rem)] sm:p-5 lg:p-8">
      ...
      {isLoading || !data ? <div className="py-12 text-center text-sm text-slate-500">Chargement du brouillon d’évaluation…</div> : <div className="grid min-h-0 min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)]">
