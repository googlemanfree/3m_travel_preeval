...
export function EvaluationDeliveryEditor({ sessionToken, sourceRecordId, open, onOpenChange, onCompleted }: Props) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="h-[100dvh] max-h-[100dvh] w-screen max-w-none overflow-x-hidden overflow-y-auto rounded-none border-0 border-slate-200 bg-slate-50/95 p-2 shadow-2xl sm:h-[100dvh] sm:max-h-[100dvh] sm:w-screen sm:rounded-none sm:p-5 lg:p-8">
      <DialogHeader className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
      ...
      {!sessionToken ? <div role="alert" className="mx-auto max-w-xl rounded-xl border border-amber-200 bg-amber-50 p-6 text-center text-sm text-amber-950"><strong className="block text-base">Session administrateur introuvable</strong><p className="mt-2">Reconnectez-vous à l’espace admin puis rouvrez ce bilan.</p></div> : isLoading || isFetching ? <div role="status" className="mx-auto flex min-h-[320px] max-w-xl items-center justify-center rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500"><span><Loader2 className="mx-auto mb-3 h-8 w-8 animate-spin text-blue-700" />Chargement du brouillon d’évaluation…</span></div> : isError || !data ? <div role="alert" className="mx-auto max-w-xl rounded-xl border border-rose-200 bg-rose-50 p-6 text-center text-sm text-rose-950"><strong className="block text-base">Le brouillon n’a pas pu être chargé</strong><p className="mt-2">{error?.message || "Aucune donnée n’a été retournée par le serveur."}</p><Button className="mt-4" variant="outline" onClick={() => { setRetryAttempts((attempts) => attempts + 1); void refetch(); }}><RefreshCw className="mr-2 h-4 w-4" />Réessayer</Button>{retryAttempts >= 3 && <Button asChild className="mt-3" variant="outline"><a href={`mailto:3mtravelandservices@gmail.com?subject=${encodeURIComponent(`Problème de chargement du bilan ${sourceRecordId}`)}&body=${encodeURIComponent(`Bonjour,\n\nLe brouillon du bilan ${sourceRecordId} ne se charge pas après ${retryAttempts} tentatives.\nURL : ${window.location.href}`)}`}><MessageCircle className="mr-2 h-4 w-4" />Signaler au support technique</a></Button>}</div> : <div className="grid min-h-0 min-w-0 gap-5 pb-24 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)]"><div className="pointer-events-none fixed left-3 top-20 z-30 rounded-full border border-slate-200 bg-white/90 px-3 py-1 text-xs text-slate-600 shadow-sm backdrop-blur sm:left-6"><span className={`mr-2 inline-block h-2 w-2 rounded-full ${autosaveStatus === "saving" ? "bg-amber-500" : autosaveStatus === "error" ? "bg-rose-500" : autosaveStatus === "saved" ? "bg-emerald-500" : "bg-slate-300"}`} />{autosaveStatus === "saving" ? "Sauvegarde automatique…" : autosaveStatus === "error" ? "Sauvegarde à vérifier" : autosaveStatus === "saved" ? "Brouillon sauvegardé" : "Édition humaine"}</div>
        <div className="min-w-0 space-y-4">
          ...
          <div className="fixed inset-x-0 bottom-16 z-40 grid grid-cols-1 gap-2 border-t border-slate-200 bg-white/95 px-3 pb-3 pt-3 shadow-[0_-8px_24px_rgba(15,23,42,0.12)] backdrop-blur sm:grid-cols-4 sm:px-6 lg:px-10"><Button variant="outline" onClick={() => void ensureDraft()} disabled={saveDraft.isPending}><Eye className="mr-1 h-4 w-4" />{saveDraft.isPending ? "Préparation…" : "Prévisualiser"}</Button><Button variant="outline" onClick={() => void openEmailPreview()} disabled={saveDraft.isPending || previewEmail.isFetching}><Mail className="mr-1 h-4 w-4" />{previewEmail.isFetching ? "Préparation…" : "Aperçu e-mail exact"}</Button><Button variant="outline" aria-busy={previewPdf.isPending} onClick={() => void openPdfPreview()} disabled={saveDraft.isPending || previewPdf.isPending}><FileText className="mr-1 h-4 w-4" />{previewPdf.isPending ? <><Loader2 className="mr-1 h-4 w-4 animate-spin" />Génération PDF…</> : "Aperçu PDF généré"}</Button><Button variant="outline" onClick={() => void saveDraftOnly()} disabled={saveDraft.isPending}><Save className="mr-1 h-4 w-4" />{saveDraft.isPending ? "Enregistrement…" : "Enregistrer le brouillon"}</Button></div>
