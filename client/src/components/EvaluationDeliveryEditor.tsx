...
export function EvaluationDeliveryEditor({ sessionToken, sourceRecordId, open, onOpenChange, onCompleted }: Props) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="h-[100dvh] max-h-[100dvh] w-screen max-w-none overflow-x-hidden overflow-y-auto rounded-none border-0 border-slate-200 bg-slate-50/95 p-2 shadow-2xl sm:h-[100dvh] sm:max-h-[100dvh] sm:w-screen sm:rounded-none sm:p-5 lg:p-8">
      ...
      {!sessionToken ? <div role="alert" className="mx-auto max-w-xl rounded-xl border border-amber-200 bg-amber-50 p-6 text-center text-sm text-amber-950"><strong className="block text-base">Session administrateur introuvable</strong><p className="mt-2">Reconnectez-vous à l’espace admin puis rouvrez ce bilan.</p></div> : isLoading || isFetching ? <div role="status" className="mx-auto flex min-h-[320px] max-w-xl items-center justify-center rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500"><span><Loader2 className="mx-auto mb-3 h-8 w-8 animate-spin text-blue-700" />Chargement du brouillon d’évaluation…</span></div> : isError || !data ? <div role="alert" className="mx-auto max-w-xl rounded-xl border border-rose-200 bg-rose-50 p-6 text-center text-sm text-rose-950"><strong className="block text-base">Le brouillon n’a pas pu être chargé</strong><p className="mt-2">{error?.message || "Aucune donnée n’a été retournée par le serveur."}</p><Button className="mt-4" variant="outline" onClick={() => void refetch()}><RefreshCw className="mr-2 h-4 w-4" />Réessayer</Button></div> : <div className="grid min-h-0 min-w-0 gap-5 pb-24 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)]">
