...
export function Candidate360Workspace({ sessionToken, candidate, onRefresh }: Props) {
  ...
  const paymentReceiptMutation = trpc.adminCandidateManagement.sendPaymentReceiptForCandidate.useMutation({
    onSuccess: async (result) => { toast.success("Reçu PDF envoyé", { description: `Le reçu avec le logo et le détail des frais a été envoyé à ${result.email}.` }); await refresh(); },
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="payments" className="space-y-3 pt-4">
          {data.payments.length ? data.payments.map((payment: any, index: number) => <div key={index} className="rounded-xl border p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><CreditCard className="h-5 w-5 text-blue-700" /><div><p className="font-semibold text-slate-900">Frais de dossier</p><p className="text-sm text-slate-500">{payment.amount?.toLocaleString("fr-FR")} {payment.currency} · {payment.method || "Méthode à préciser"}</p></div></div><StateBadge status={String(payment.status).toLowerCase()} /></div><p className="mt-3 text-sm text-slate-600">Référence : {payment.reference || "Non renseignée"} · Date : {formatDate(payment.paidAt)}</p>{String(payment.status).toUpperCase() === "SUCCESS" && <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-100 bg-blue-50/60 p-3"><p className="text-xs text-blue-950">Le reçu PDF professionnel comprend le logo, le montant et le détail des frais d’ouverture, de traitement et de soumission.</p><Button type="button" size="sm" className="gap-2 bg-blue-700 text-white hover:bg-blue-800" disabled={paymentReceiptMutation.isPending} onClick={() => paymentReceiptMutation.mutate({ sessionToken, candidateId: candidate.id, deliveryMode: "resend" })}>{paymentReceiptMutation.isPending ? <><Loader2 className="h-4 w-4 animate-spin" />Envoi…</> : <><Mail className="h-4 w-4" />Envoyer le reçu PDF</>}</Button></div>}</div>) : <p className="rounded-lg border border-dashed p-4 text-sm text-slate-500">Aucun paiement relié à ce dossier agence.</p>}
        </TabsContent>
