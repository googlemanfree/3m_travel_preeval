...
export function Candidate360Workspace({ sessionToken, candidate, onRefresh }: Props) {
  ...
  const evaluationReminderMutation = trpc.unifiedRequests.sendEvaluationReminder.useMutation({
    onSuccess: async (result) => { toast.success("Relance envoyée", { description: result.message }); await refresh(); },
  ...
  const paymentReceiptMutation = trpc.adminCandidateManagement.sendPaymentReceiptForCandidate.useMutation({
    onSuccess: async (result) => { toast.success("Reçu PDF envoyé", { description: `Le reçu signé avec le logo et les mentions de frais a été envoyé à ${result.email}.` }); await refresh(); },
    onError: (mutationError) => toast.error("Envoi du reçu impossible", { description: mutationError.message }),
  });
