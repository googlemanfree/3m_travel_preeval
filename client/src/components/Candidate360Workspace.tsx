      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as Props["initialTab"])} className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4 pt-4">
          ...
          {data.candidateJourney?.steps?.length > 0 && <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:p-6">
            ...
            <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{data.candidateJourney.steps.map((step: any) => <div key={step.id} className={`rounded-lg border p-3 ${step.state === "completed" ? "border-emerald-200 bg-emerald-50" : step.state === "current" ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-slate-50"}`}><div className="flex items-start justify-between gap-2"><p className="text-sm font-semibold text-slate-900">{step.index + 1}. {step.label}</p><span className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{step.state === "completed" ? "Terminé" : step.state === "current" ? "En cours" : "Verrouillé"}</span></div><p className="mt-1 text-xs leading-5 text-slate-600">{step.description}</p>{step.sourceUrl && <a href={step.sourceUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex text-xs font-semibold text-blue-700 underline">Source officielle</a>}{step.state === "current" && <Button type="button" size="sm" variant="outline" className="mt-3 w-full border-blue-300 bg-white text-blue-800 hover:bg-blue-50" disabled={journeyStepMutation.isPending || actionLocks.journeyStep} onClick={() => { lockAction("journeyStep"); journeyStepMutation.mutate({ sessionToken, candidateId: candidate.id, stepId: `checklist-${step.index}`, checked: true }); }}>{journeyStepMutation.isPending ? "Validation…" : "Marquer l’étape comme faite"}</Button>}</div>)}</div>
