...
export function Candidate360Workspace({ sessionToken, candidate, onRefresh }: Props) {
  ...
  const [agreementContent, setAgreementContent] = useState("Le présent protocole formalise la demande d’accompagnement administratif et de mobilité internationale.\n\nLe candidat s’engage à transmettre des informations exactes, complètes et à jour et à fournir les pièces demandées dans les délais indiqués.\n\nLes frais d’accompagnement sont distincts des frais officiels et 3M Travel & Services ne garantit ni visa, ni permis, ni emploi, ni décision d’une autorité ou d’un employeur.\n\nLa signature est possible uniquement après confirmation du paiement et confirme la lecture du présent protocole.");
  ...
  const offlineEvaluationMutation = trpc.adminCandidateManagement.validateOfflineEvaluation.useMutation({
    ...
    onError: (mutationError) => toast.error("Validation hors ligne impossible", { description: mutationError.message }),
  });
  const agreementProtocolMutation = trpc.adminCandidateManagement.sendAgreementProtocol.useMutation({
    onSuccess: async (result) => { toast.success("Protocole envoyé", { description: `Déposé dans l’espace client et envoyé par e-mail pour ${result.dossierNumber}.` }); await refresh(); },
    onError: (mutationError) => toast.error("Envoi du protocole impossible", { description: mutationError.message }),
  });
  const approvePaymentReceiptMutation = trpc.adminCandidateManagement.approvePaymentReceipt.useMutation({
    ...
    onError: (mutationError) => toast.error("Validation du reçu impossible", { description: mutationError.message }),
  });
