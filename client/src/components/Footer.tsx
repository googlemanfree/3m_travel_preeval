...
const SOCIAL_LINKS: SocialLink[] = [
  ...
  { key: "x", icon: Twitter, href: "https://twitter.com/3mtravelagency", label: { fr: "X", en: "X" }, description: { fr: "Ouvrir le compte X dans un nouvel onglet.", en: "Open the X account in a new tab." }, color: "hover:text-sky-300" },
];
...
function FooterShortcut({ link, language, onTrack }: FooterShortcutProps) {
  ...
  return <span className="group relative inline-flex max-w-full"><Link href={link.href} aria-describedby={descriptionId} onClick={() => onTrack(link)} className={FOOTER_SHORTCUT_CLASS}><span>{link.label[language]}</span><span aria-hidden="true" className="ml-1 text-blue-200 opacity-0 transition-opacity duration-150 group-hover:opacity-100 group-focus-visible:opacity-100 motion-reduce:transition-none">↗</span></Link><span id={descriptionId} role="tooltip" className={FOOTER_TOOLTIP_CLASS}>{link.description[language]}</span></span>;
}
