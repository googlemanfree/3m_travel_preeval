                  ...
                  <div className="w-full rounded-xl border border-red-900/60 bg-red-950/20 p-3">
                    ...
                    {activeAnnotationDocumentId === document.id && (
                      <div className="grid gap-2 md:grid-cols-[180px_minmax(0,1fr)_auto]">
                        <input value={annotationArea} onChange={(event) => setAnnotationArea(event.target.value)} placeholder="Zone : photo, MRZ..." className="h-10 rounded-xl border border-red-800 bg-slate-950 px-3 text-xs text-white placeholder:text-slate-500" aria-label="Zone concernée par la correction" maxLength={100} />
                        <textarea value={annotationMessage} onChange={(event) => setAnnotationMessage(event.target.value)} placeholder="Décrivez précisément la correction à effectuer..." className="min-h-10 rounded-xl border border-red-800 bg-slate-950 px-3 py-2 text-xs text-white placeholder:text-slate-500" aria-label="Description de la correction" maxLength={1000} />
