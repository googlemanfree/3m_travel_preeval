    ...
    <section className="space-y-5" aria-labelledby="evaluation-review-queue-title">
      <div className="flex flex-col justify-between gap-3 rounded-2xl bg-gradient-to-r from-amber-50 via-white to-blue-50 p-5 ring-1 ring-amber-100 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Les brouillons IA attendent votre relecture obligatoire avant toute programmation ou diffusion.</p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {rows.map((row) => <Card key={row.id} className="border-amber-100 shadow-sm"><CardContent className="space-y-4 p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs font-semibold text-blue-700">{row.dossierNumber}</p><h3 className="mt-1 font-bold text-slate-950">{row.fullName}</h3><p className="mt-1 text-sm text-slate-600">{row.destination || "Destination à confirmer"} · Score proposé : {row.scoringTotal ?? "—"}/100</p></div><Badge variant="outline" className="border-amber-200 bg-amber-50 text-amber-800">À relire</Badge></div><div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3 text-xs text-slate-500"><span>Mis à jour le {new Date(row.updatedAt).toLocaleString("fr-FR")}</span><Button type="button" size="sm" onClick={() => onOpenDossier(row.dossierNumber)}><FilePenLine className="mr-2 h-4 w-4" />Traiter le bilan</Button></div></CardContent></Card>)}
      </div>
    </section>
  );
}
