...
type Draft = { heroTitle: string; heroDescription: string; serviceIntro: string; requestIntro: string; serviceDefinitionsJson: string };
...
export default function AdminDigitalContentEditor({ sessionToken }: Props) {
  ...
  useEffect(() => { if (data) setDraft({ heroTitle: data.heroTitle, heroDescription: data.heroDescription, serviceIntro: data.serviceIntro, requestIntro: data.requestIntro, serviceDefinitionsJson: data.serviceDefinitionsJson }); }, [data]);
