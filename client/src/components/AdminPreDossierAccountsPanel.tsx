...
export default function AdminPreDossierAccountsPanel({ sessionToken }: { sessionToken: string }) {
  ...
  const activateMutation = trpc.adminCandidateManagement.activatePreDossierAccount.useMutation({
    onSuccess: (result) => {
      toast({ title: "Dossier activé", description: result.emailSent ? "Le dossier est actif dans l’espace client et l’e-mail a été envoyé." : "Le dossier est actif dans l’espace client ; l’e-mail devra être relancé." });
