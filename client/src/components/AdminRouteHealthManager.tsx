    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="404-config" className="w-full">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {links.map((link, index) => <div key={`${link.label}-${index}`} className="flex flex-col sm:flex-row gap-2 rounded-lg border p-2"><Input aria-label={`Libellé du lien ${index + 1}`} value={link.label} onChange={(event) => setLinks((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item))} maxLength={100} /><Input aria-label={`URL du lien ${index + 1}`} value={link.href} onChange={(event) => setLinks((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, href: event.target.value } : item))} maxLength={500} /><Button variant="ghost" size="icon" aria-label={`Supprimer le lien ${link.label}`} onClick={() => setLinks((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="w-4 h-4 text-red-600" /></Button></div>)}
                </div>
                <div className="grid gap-2 md:grid-cols-[1fr_2fr_auto]"><Input placeholder="Libellé" value={newLinkLabel} onChange={(event) => setNewLinkLabel(event.target.value)} maxLength={100} /><Input placeholder="/evaluation ou https://..." value={newLinkHref} onChange={(event) => setNewLinkHref(event.target.value)} maxLength={500} /><Button type="button" variant="outline" onClick={add404Link} className="gap-2"><Plus className="w-4 h-4" />Ajouter</Button></div>
              </div>
        ...
        <TabsContent value="404-events" className="space-y-4 mt-4">
          <Card><CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><CardTitle className="text-base">Chemins introuvables les plus récents</CardTitle><Input className="sm:max-w-xs" placeholder="Filtrer un chemin" value={errorSearch} onChange={(event) => setErrorSearch(event.target.value)} maxLength={200} /></div></CardHeader><CardContent>
        ...
        <TabsContent value="external-links" className="space-y-4 mt-4">
          <Card><CardHeader><CardTitle className="text-base">Ajouter un lien à surveiller</CardTitle></CardHeader><CardContent><div className="grid gap-2 md:grid-cols-[1fr_2fr_auto]"><Input placeholder="Nom du portail" value={externalLabel} onChange={(event) => setExternalLabel(event.target.value)} maxLength={100} /><Input placeholder="https://portail-officiel.example" value={externalUrl} onChange={(event) => setExternalUrl(event.target.value)} maxLength={500} /><Button onClick={() => addExternalLink.mutate({ sessionToken, url: externalUrl, label: externalLabel })} disabled={addExternalLink.isPending} className="gap-2"><Plus className="w-4 h-4" />Ajouter</Button></div></CardContent></Card>
