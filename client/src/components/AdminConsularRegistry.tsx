    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
          <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Pays, procédure, région ou guide…" className="h-10 bg-slate-50 pl-9 dark:bg-slate-950" maxLength={200} /></div>
      ...
      <Dialog open={Boolean(editingEntry)} onOpenChange={(open) => { if (!open) setEditingEntry(null); }}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid gap-4 py-2">
            <div className="grid gap-2"><Label htmlFor="portal-url">URL officielle</Label><Input id="portal-url" type="url" value={portalUrl} onChange={(event) => setPortalUrl(event.target.value)} placeholder="https://…" maxLength={500} /></div>
            <div className="grid gap-2"><Label htmlFor="portal-label">Libellé du portail</Label><Input id="portal-label" value={portalLabel} onChange={(event) => setPortalLabel(event.target.value)} placeholder="Ex. Portail officiel des visas" maxLength={100} /></div>
            <div className="grid gap-2"><Label htmlFor="portal-date">Date de vérification</Label><Input id="portal-date" value={verifiedAt} onChange={(event) => setVerifiedAt(event.target.value)} placeholder="Ex. 19 août 2026" maxLength={50} /></div>
            ...
            <div className="grid gap-2"><Label htmlFor="portal-note">Note de contrôle</Label><Textarea id="portal-note" value={verificationNote} onChange={(event) => setVerificationNote(event.target.value)} placeholder="Source consultée, motif de la mise à jour ou point à recontrôler…" maxLength={2000} /></div>
          </div>
