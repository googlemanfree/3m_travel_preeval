...
- [x] Vérifier le flux complet depuis l’interface admin sur un dossier candidat réel, avec confirmation explicite avant l’envoi.
...
- [x] Préparer le bilan complet des sept points prioritaires après réception et traitement des points restants.
...
- [x] Point 6 — Unifier le pilotage en ligne/agence, rattacher sans doublon et enrichir le formulaire de pré-dossier agence.
- [x] Point 7 — Faire retourner un vrai 404 pour /evaluation-primaire au lieu de rediriger vers #evaluation-multi.
- [x] Tester et publier chaque point validé ; ne supprimer aucune route, donnée ou composant sans confirmation.
...
- [x] Publier le correctif et fournir l’URL/action/résultat exacts.
...
## Audit officiel par lots — Türkiye
- [x] Contrôler les sources institutionnelles turques pour visiteur, études, résidence et travail.
...
## Corrections issues de l’audit des routes
...
- [x] Ajouter les régressions UI/serveur et effectuer un clic réel non destructif en session admin, sans toucher à un paiement réel. Les régressions et TypeScript passent ; un clic live sur « Actualiser » et l’ouverture de « Pilotage synchronisé » ont été exécutés avec succès, documentés dans `docs/admin-action-lock-live-check-2026-09-09.md`.

## Contrôle paiement agence, recherche et inventaire comptes de test — priorité utilisateur
- [x] Vérifier que le toast post-paiement signifie bien paiement confirmé mais protocole client encore bloquant, sans relancer de validation réelle. Les tests de gating et l’observation live du dossier fictif confirment ce comportement normal.
- [x] Diagnostiquer le rattachement de l’évaluation hors ligne pour `3M-AGN-270002` sans modifier son statut ni son paiement. Le compte `candidates.id=810001` correspond par e-mail ; le code normalise désormais les e-mails et propose un fallback agence traçable.
...
- [x] Renforcer le bloc « Action suivante » avec condition bloquante, responsable et action directe.
- [x] Rendre l’historique/journal d’activité plus visible directement dans Candidate360.
...
- [x] Rendre les 17 étapes du Parcours synchronisé actionnables par l’admin avec séquence et prérequis serveur.
- [x] Auditer et corriger le flux de signature du protocole côté client et admin, avec paiement confirmé obligatoire.
- [x] Permettre le changement de destination côté admin avec synchronisation vers l’espace client et traçabilité.
...
- [x] Synchroniser et relire le commit db34d9e
- [x] Vérifier le tri des avis par note et date avec réinitialisation de pagination
- [x] Vérifier le bouton Réessayer de ReviewsErrorBoundary sans rechargement complet
- [x] Vérifier AggregateRating uniquement avec les statistiques approuvées et absence de JSON-LD sans avis approuvé
...
- [x] Synchroniser et relire le commit e599202
- [x] Vérifier les routes /en, /en/procedures/:countryId et /en/destinations/:countryId
- [x] Vérifier le rendu locale-aware de CountryDetailPage et les quatre slugs anglais
- [x] Vérifier procedureMetaForPathEn, les titles/H1/meta descriptions et og:locale/inLanguage conditionnels
- [x] Vérifier HREFLANG_PAIRS avec alternates fr/en/x-default sur les pages concernées
- [x] Vérifier getIndexablePublicPaths et le sitemap avec les pages EN
...
- [ ] Synchroniser et relire af92e4b et 496ec07
- [ ] Vérifier que ProofGallerySection référence bien 10 images
- [ ] Contrôler visuellement les 7 nouvelles images : caviardage, filigrane et absence de données personnelles lisibles
- [ ] Vérifier les métadonnées évidentes des 7 nouvelles images
- [ ] Exécuter TypeScript et les tests ciblés
- [ ] Vérifier visuellement les 10 photos dans la galerie
