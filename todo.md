...
- [x] Auditer les rôles, permissions et protections serveur actuellement utilisés
- [x] Garantir qu’un seul rôle admin commun donne les mêmes accès à tous les administrateurs
...
- [x] Ajouter un brouillon de bilan modifiable et une prévisualisation complète avant envoi définitif
- [x] Permettre l’ajustement administrateur du score, du verdict, des forces, axes d’amélioration et recommandations avec traçabilité
...
## Amélioration Interface Administrateur Tourisme & Devis (v251)
- [x] Ajouter un système de sélection multiple (batch) pour valider ou changer le statut de plusieurs demandes de tourisme en un clic
- [x] Enrichir les détails de la modale admin avec la liste complète des services, catégories de véhicules/hôtels et notes d’IA
- [x] Permettre l'envoi direct de notification e-mail ou WhatsApp au client lors de la fixation d'un devis validé
- [x] Tester les nouvelles actions et publier l’expérience admin améliorée

## Calendrier Interactif des Réservations Confirmées (v252)
- [x] Ajouter la procédure tRPC `listConfirmedReservations` pour extraire les réservations et voyages confirmés
- [x] Créer le composant `AdminCalendarView.tsx` avec affichage chronologique, filtres par type (hôtel, véhicule, pack) et modale de détail
- [x] Intégrer le calendrier en tant qu'onglet dédié dans le tableau de bord administrateur (`AdminDashboard.tsx`)
- [x] Valider avec les tests unitaires et TypeScript puis publier la version

## Correction et Synchronisation des Réservations & Paiements (v255)
- [x] Rendre le bouton de confirmation de réservation pleinement actif et synchrone avec l’espace administrateur
- [x] Ajouter l’action client « Valider » et le suivi des PNR / documents originaux dans l’espace client
- [x] Intégrer la saisie du mode de paiement (Agence ou Orange Money) et de l’ID de transaction pour validation automatique
- [x] Valider avec les tests unitaires et TypeScript puis publier la version corrigée

## Envoi de Reçu de Paiement par E-mail depuis le Tableau Admin (v257)
- [ ] Ajouter la procédure tRPC `sendPaymentReceiptEmail` pour envoyer le reçu de paiement et la quittance au client
- [ ] Ajouter le bouton « Envoyer le reçu » dans le tableau admin `AdminReservationPayments.tsx` avec modale de confirmation
...
- [x] Intégrer le bouton de téléchargement du reçu dans la liste des réservations de l’espace personnel du candidat (`MonEspace.tsx` ou similaire)
- [x] Valider avec les tests unitaires et TypeScript puis publier la version finale
