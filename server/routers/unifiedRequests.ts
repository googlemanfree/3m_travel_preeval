...
initializeEvaluationDelivery: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const [linkedAgencyDossier] = await db.select({ id: agencyDossiers.id }).from(agencyDossiers)
        .where(and(isNull(agencyDossiers.deletedAt), eq(agencyDossiers.email, candidate.email)))
        .orderBy(desc(agencyDossiers.createdAt)).limit(1);
      if (linkedAgencyDossier) {
        const agencyApplication = await resolveEvaluationApplication(db, linkedAgencyDossier.id);
        if (agencyApplication) return { success: true, created: false, candidateId: candidate.id, application: agencyApplication, message: "Dossier d’évaluation agence résolu pour ce compte candidat." };
      }
  ...
  getEvaluationDelivery: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!application) throw new TRPCError({ code: "NOT_FOUND", message: "Aucun brouillon de bilan n’est disponible : le compte peut être lié à une évaluation hors ligne sans CV exploitable ou sans pré-dossier agence résoluble. Vérifiez le CV et le rattachement du pré-dossier avant de réessayer." });
      let details: Record<string, any> = {};
      try { details = JSON.parse(application.scoringDetails || "{}"); } catch { details = {}; }
      const adminDraft = details.adminDraft ?? {};
      const agencyDossierCandidates = await db.select({ email: agencyDossiers.email, evaluationValidatedAt: agencyDossiers.evaluationValidatedAt, evaluationValidatedBy: agencyDossiers.evaluationValidatedBy, createdAt: agencyDossiers.createdAt }).from(agencyDossiers).where(isNull(agencyDossiers.deletedAt)).orderBy(desc(agencyDossiers.createdAt)).limit(200);
      const normalizedApplicationEmail = String(application.email || "").trim().toLowerCase();
      const offlineAgencyDossier = agencyDossierCandidates.find((item) => String(item.email || "").trim().toLowerCase() === normalizedApplicationEmail);
