...
async function loadSourceSnapshots(): Promise<SourceSnapshot[]> {
  ...
  return [
    ...
    ...tourism.map((row) => ({ sourceType: "tourism" as const, sourceRecordId: row.id, candidateId: row.candidateId, displayReference: row.requestRef, fullName: "Client Tourisme", email: row.candidateEmail, phone: row.candidatePhone ?? null, destination: row.destination, requestTypeLabel: `Tourisme (${row.serviceType})`, sourceStatus: row.status, evaluationApprovalStatus: null, evaluationDeliveryStatus: null, evaluationReportViewedAt: null, createdAt: row.createdAt, updatedAt: row.updatedAt })),
  ];
}
