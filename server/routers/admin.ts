...
export function normalizeAdminDocumentType(value: unknown, fileName?: unknown): (typeof ADMIN_DOCUMENT_TYPES)[number] {
  ...
  const aliases: Record<string, (typeof ADMIN_DOCUMENT_TYPES)[number]> = {
    ...
    autre: "autre", autres: "autre", other: "autre", professional_documents: "autre",
  };
...
export const adminRouter = router({
  ...
  approveDocument: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), documentId: z.number().int().positive(), comment: z.string().optional() }))
    .mutation(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
  ...
  getUserDetailsWithDocuments: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), userId: z.number() }))
    .query(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
  ...
  validateAndSendBilan: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), bilanId: z.number().int().positive() }))
    .mutation(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
  ...
  rejectBilan: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), bilanId: z.number().int().positive(), reason: z.string().min(1).max(2000) }))
    .mutation(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
  ...
  updateApplicationStatus: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), applicationId: z.number().int().positive(), status: z.string().min(1).max(50) }))
    .mutation(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
  ...
  getApplicationDetails: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), applicationId: z.number().int().positive() }))
    .query(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
      ...
      try {
        ...
        const candidateRows = await db
          .select({
            ...
            evaluationReviewedAt: candidates.evaluationReviewedAt,
            evaluationReviewedBy: candidates.evaluationReviewedBy,
            evaluationReviewNote: candidates.evaluationReviewNote,
        ...
        const normalizedOnline = onlineApps.map(app => ({
          ...
          evaluationReviewedAt: candidateByEmail.get(app.email.toLowerCase())?.evaluationReviewedAt ?? null,
          evaluationReviewedBy: candidateByEmail.get(app.email.toLowerCase())?.evaluationReviewedBy ?? null,
          evaluationReviewNote: candidateByEmail.get(app.email.toLowerCase())?.evaluationReviewNote ?? null,
        ...
        const normalizedAccounts = candidateRows
          ...
          .map((candidate) => ({
            ...
            evaluationReviewedAt: candidate.evaluationReviewedAt ?? null,
            evaluationReviewedBy: candidate.evaluationReviewedBy ?? null,
            evaluationReviewNote: candidate.evaluationReviewNote ?? null,
            evaluationValidatedAt: candidate.evaluationReviewedAt ?? null,
            evaluationValidatedBy: candidate.evaluationReviewedBy ?? null,
  ...
  updateCandidateStatus: publicProcedure
    .input(z.object({
      sessionToken: z.string().min(1).max(512),
      candidateId: z.string().min(1).max(50), // Format: "online_123" ou "agency_456"
  ...
  revertCandidateStatus: publicProcedure
    .input(z.object({
      ...
      candidateId: z.string().min(1).max(50),
  ...
  listDestinationDocumentsAdmin: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), search: z.string().max(200).optional() }))
    .query(async ({ input }) => {
      await requireValidAdminSession(input.sessionToken);
  ...
  deleteDestinationDocumentAdmin: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), id: z.number() }))
    .mutation(async ({ input }) => {
      await requireValidAdminSession(input.sessionToken);
        ...
        if (accountMatch) {
          ...
          return serializeAdminCandidateDetails({
            ...
            candidate: {
              ...
              evaluationValidatedAt: account.evaluationReviewedAt,
              evaluationValidatedBy: account.evaluationReviewedBy,
        ...
        if (source === "online") {
          ...
          return serializeAdminCandidateDetails({
            ...
            candidate: {
              ...
              scoringData,
              avatarUrl,
              evaluationDeclarationStatus: candRec?.evaluationDeclarationStatus ?? "not_declared",
              evaluationDeclaredAt: candRec?.evaluationDeclaredAt ?? null,
              evaluationReviewedAt: candRec?.evaluationReviewedAt ?? null,
              evaluationReviewedBy: candRec?.evaluationReviewedBy ?? null,
              evaluationReviewNote: candRec?.evaluationReviewNote ?? null,
              evaluationValidatedAt: candRec?.evaluationReviewedAt ?? null,
              evaluationValidatedBy: candRec?.evaluationReviewedBy ?? null,
              createdAt: app.createdAt,
              updatedAt: app.updatedAt,
            },
            documents: docs,
          ...
          return serializeAdminCandidateDetails({
            ...
            candidate: {
              ...
              scoringData: null,
              avatarUrl,
              evaluationDeclarationStatus: dossier.evaluationValidatedAt ? "validated" : "not_declared",
              evaluationDeclaredAt: null,
              evaluationReviewedAt: dossier.evaluationValidatedAt ?? null,
              evaluationReviewedBy: dossier.evaluationValidatedBy ?? null,
              evaluationReviewNote: null,
              evaluationValidatedAt: dossier.evaluationValidatedAt ?? null,
              evaluationValidatedBy: dossier.evaluationValidatedBy ?? null,
      ...
      try {
        const [clientRows, candidateRows, agencyDocumentRows] = await Promise.all([
          db.select({
            ...
            applicationCandidateId: applications.candidateId,
            applicationDossierNumber: applications.dossierNumber,
          }).from(clientDocuments).leftJoin(evaluations, eq(clientDocuments.evaluationId, evaluations.id)).leftJoin(applications, eq(clientDocuments.evaluationId, applications.id)).orderBy(desc(clientDocuments.receiptGeneratedAt)).limit(1000),
          db.select({
  ...
  resendFailedEmail: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), logId: z.number().int().positive() }))
    .mutation(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB non disponible" });
  ...
  emailDeliveryIncidentsHistory: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB non disponible" });
      return db.select().from(emailDeliveryIncidents).orderBy(desc(emailDeliveryIncidents.createdAt)).limit(200);
    }),
  ...
  getCandidate360: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [requirements, operationalDocuments, legacyDocuments, tasks, notes, statusHistory, activityLogs, notifications, messages, documentClarifications, documentClarificationEventHistory, advisors, requestHistory, latestEvaluations] = await Promise.all([
        safeCollection(db.select().from(documentRequirements).where(eq(documentRequirements.caseId, operationalCase.id)).orderBy(asc(documentRequirements.requestedAt)).limit(100), [], "requirements"),
        safeCollection(db.select().from(caseDocuments).where(eq(caseDocuments.caseId, operationalCase.id)).orderBy(desc(caseDocuments.uploadedAt)).limit(100), [], "caseDocuments"),
        linkedApplication
          ...
          : Promise.resolve([]),
        safeCollection(db.select().from(caseTasks).where(eq(caseTasks.caseId, operationalCase.id)).orderBy(asc(caseTasks.dueAt)).limit(100), [], "tasks"),
        safeCollection(db.select().from(caseAdminNotes).where(eq(caseAdminNotes.caseId, operationalCase.id)).orderBy(desc(caseAdminNotes.createdAt)).limit(100), [], "notes"),
        safeCollection(db.select().from(caseStatusHistory).where(eq(caseStatusHistory.caseId, operationalCase.id)).orderBy(desc(caseStatusHistory.createdAt)).limit(200), [], "statusHistory"),
        safeCollection(db.select().from(caseActivityLogs).where(eq(caseActivityLogs.caseId, operationalCase.id)).orderBy(desc(caseActivityLogs.createdAt)).limit(200), [], "activityLogs"),
        ...
        safeCollection(db.select({ id: adminAccounts.id, fullName: adminAccounts.fullName, email: adminAccounts.email, adminType: adminAccounts.adminType }).from(adminAccounts).where(eq(adminAccounts.status, "active")).orderBy(asc(adminAccounts.fullName)).limit(50), [], "advisors"),
      ...
      return {
        ...
        documents: [
          ...
          ...((reference.source === "online" && (sourceRecord as typeof applications.$inferSelect).cvUrl && ![...operationalDocuments, ...legacyDocuments].some((document: any) => normalizeAdminDocumentType(document.documentType, document.documentName ?? document.fileName) === "cv")) ? [{
            ...
            documentUrl: (sourceRecord as typeof applications.$inferSelect).cvUrl,
            uploadedAt: sourceRecord.createdAt,
  ...
  listAdvisorTreatmentDeadlines: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [openCases, openTasks, activeAdmins, requestRows] = await Promise.all([
        ...
        db.select({ id: adminAccounts.id, fullName: adminAccounts.fullName, email: adminAccounts.email }).from(adminAccounts).where(eq(adminAccounts.status, "active")).limit(50),
