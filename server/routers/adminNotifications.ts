...
export const adminNotificationsRouter = router({
  ...
  list: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), limit: z.number().min(1).max(100).default(30) }))
    .query(async ({ input }) => {
      const admin = await requireValidAdminSession(input.sessionToken);
      const db = await getDb();
  ...
  markAsRead: publicProcedure
    .input(z.object({ sessionToken: z.string().min(1).max(512), notificationId: z.number().int().positive() }))
