...
export async function getOrCreateCandidateForPlatformUser(user: { id: number; name: string | null; email: string | null }) {
  ...
  await db.insert(candidates).values({
    ...
    email,
    // Compte créé via connexion sociale (Google/Facebook) : pas de mot de passe local.
    passwordHash: "",
    emailVerified: true, // déjà vérifié par le fournisseur OAuth
...
export const candidateRouter = router({
  ...
  getProfile: candidateProcedure.query(async ({ ctx }) => {
    const c = ctx.candidate as unknown as Candidate;
    return {
      id: c.id,
      fullName: c.fullName,
      email: c.email,
  ...
  updateProfile: candidateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(candidates).set(updateData).where(eq(candidates.id, candidate.id));
      return { success: true };
    }),
  ...
  listDocuments: candidateProcedure.query(async ({ ctx }) => {
    ...
    const files = await db
      .select()
      .from(candidateFiles)
      .where(eq(candidateFiles.candidateId, (ctx.candidate as Candidate).id))
  ...
  saveDocument: candidateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (candidate.dossierStatus === "nouveau") {
        await db.update(candidates)
  ...
  deleteDocument: candidateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db
        ...
        .where(and(eq(candidateFiles.id, input.fileId), eq(candidateFiles.candidateId, candidate.id)));
