...
describe("Chargement frais après déploiement", () => {
  it("force la recherche d’un nouveau service worker sans attendre le cache HTTP", () => {
    ...
    const versionedWorker = fs.readFileSync(path.join(projectRoot, "client/public/sw-v9.js"), "utf8");
    ...
    expect(source).toContain("/sw-v9.js?revision=");
    expect(versionedWorker).toContain("3m-travel-pwa-v9-transparency-footer");
