...
const serviceWorker = readFileSync(resolve(import.meta.dirname, "../client/public/sw-v8.js"), "utf8");
...
describe("révision PWA du poste administrateur", () => {
  it("enregistre une révision de service worker distincte pour évacuer les bundles périmés", () => {
    expect(indexHtml).toContain("2026-08-24-public-free-evaluation");
    expect(indexHtml).toContain("/sw-v8.js?revision=");
  ...
  it("préserve le réseau d’abord pour les navigations et purge les anciens caches", () => {
    expect(serviceWorker).toContain("3m-travel-pwa-v8-public-free-evaluation");
