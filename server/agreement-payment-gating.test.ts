...
describe("agreement and payment gating contracts", () => {
  ...
  it("keeps online payment confirmation behind the agreement gate", () => {
    ...
    expect(paymentSource).toContain('dossierStatus: application.agreementSigned && application.evaluationDeliveryStatus === "sent" && Boolean(application.evaluationClientConfirmedAt) ? "paye" : application.dossierStatus');
    expect(webhookSource).toContain('paymentStatus === "SUCCESS" && application.agreementSigned && application.evaluationDeliveryStatus === "sent" && Boolean(application.evaluationClientConfirmedAt) ? "paye" : application.dossierStatus');
  });
